...
# --- ghq + fzf: gitcd でリポジトリを検索して移動 ---
gitcd() {
  local dir
  dir=$(ghq list --full-path | fzf --prompt="ghq> " --height=40% --reverse \
        --preview "git -C {} log --oneline -10 2>/dev/null || ls -la {}" \
        --query "$*" --select-1 --exit-0) || return
  [ -n "$dir" ] && cd "$dir"
}

# --- プロンプトにカレントディレクトリの完全パスを表示 ---
# robbyrussell テーマの %c (カレントディレクトリ名のみ) を %~ (完全パス) に変更
# ホームディレクトリを ~ ではなく /home/soh と表示したい場合は %~ を %d に変える
# 1行目: カレントディレクトリの完全パス（+ git ブランチ）
# 2行目: プロンプト記号のみ（ここにコマンドを打つ）
